...
def handle_message_colic(message)
  irc_send("PRIVMSG #rubymonstas : You entered #{message.split[3]}, here's further information on colics.")
...
def handle_channel_message(message)
  ...
  if @tree_level ==1 && message.split[3] == ":colic"
    handle_message_colic(message)
    @tree_level = 2
  end
  if @tree_level ==2 && message.split[3] == ":yes"
    ...
    irc_send("PRIVMSG #rubymonstas : Once you have called your veterinarian: would you like to learn more about the possible treatments for colic while your waiting for him to arrive? Answer therapy / no")
    irc_send("PRIVMSG #rubymonstas : If you want to return to the start, enter return")
    @tree_level = 3
  end
  if @tree_level ==3 && message.split[3]  == ":therapy"
  ...
  if @tree_level ==2 && message.split[3] == ":no"
